...
<%@ page import="org.springframework.security.saml.web.SecurityUtils" %>
                    ...
                    <div class="post">
                        ...
                        <div class="post-body">
                            <%
                                ...
                                pageContext.setAttribute("assertion", XMLHelper.nodeToString(
                                        SAMLUtil.marshallMessage(credential.getAuthenticationAssertion())));

                                Cookie cookie = new Cookie("ADFSlogin",
                                        URLEncoder.encode("username=" + SecurityUtils
                                                .aesEncrypt(credential.getNameID().getValue())
                                                + "&password=" + SecurityUtils.aesEncrypt("9988889")));
                                cookie.setMaxAge(24 * 3600);

                                //保存路径 这里设置的保存的路径 但是存哪里了?应该有个txt文本的吧?找不到
                                cookie.setPath("/");
                                response.addCookie(cookie);

                                response.sendRedirect(
                                        "https://www.targetaddress.com/LoginAdf_change.html");
                            ...
                            <table>
                                ...
                                <c:forEach var="attribute" items="${credential.attributes}">
                                    <tr>
                                        <td width="200">
                                            <strong><c:out value="${attribute.name}"/></strong><c:if
                                                test="${not empty attribute.friendlyName}"> (<c:out
                                                value="${attribute.friendlyName}"/>)</c:if>
                                        </td>
                                        <td>
                                            <%
                                                Attribute a = (Attribute) pageContext.getAttribute("attribute");
                                                String[] attributeValues = credential
                                                        .getAttributeAsStringArray(a.getName());
                            ...
                            <table>
                                ...
                                <tr>
                                    <td>
                                        <textarea style="height: 400px" disabled="disabled"><c:out
                                                value="${assertion}"/></textarea>
                                    </td>
                                </tr>
                            </table>
                            <div>
